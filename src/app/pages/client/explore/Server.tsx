...
const useRoomTypeFilters = (): RoomTypeFilter[] => {
  ...
  return useMemo(
    () => [
      {
        title: t('pages:client.explore.all'),
        value: undefined,
      },
      {
        title: t('pages:client.explore.spaces'),
        value: RoomType.Space,
      },
      {
        title: t('pages:client.explore.rooms'),
        value: 'null',
      },
    ],
    [t]
...
function Search({ active, loading, searchInputRef, onSearch, onReset }: SearchProps) {
  ...
  return (
    <Box as="form" direction="Column" gap="100" onSubmit={handleSearchSubmit}>
      ...
      <Text size="L400">{t('pages:client.explore.search')}</Text>
      <Input
        ref={searchInputRef}
        ...
        size="500"
        variant="Background"
        placeholder={t('pages:client.explore.search_for_keyword')}
        before={
          active && loading ? (
        ...
        after={
          active ? (
            <Chip
              ...
              onClick={onReset}
            >
              <Text size="B300">{t('pages:client.explore.clear')}</Text>
            </Chip>
          ) : (
            <Chip type="submit" variant="Primary" size="400" radii="Pill" outlined>
              <Text size="B300">{t('pages:client.explore.enter')}</Text>
            </Chip>
          )
        }
      />
          ...
          <Menu variant="Surface">
            ...
            >
              <Text style={{ padding: config.space.S100 }} size="L400" truncate>
                {t('pages:client.explore.protocols')}
              </Text>
          ...
          <Menu variant="Surface">
            <Box direction="Column" gap="400" style={{ padding: config.space.S300 }}>
              <Box direction="Column" gap="100">
                <Text size="L400">{t('pages:client.explore.presets')}</Text>
              ...
              <Box as="form" onSubmit={handleLimitSubmit} direction="Column" gap="300">
                <Box direction="Column" gap="100">
                  <Text size="L400">{t('pages:client.explore.custom_limit')}</Text>
                  <Input
                    name="limitInput"
                    size="300"
                    variant="Background"
                    ...
                    type="number"
                    radii="400"
                    aria-label={t('pages:client.explore.per_page_item_limit')}
                  />
                </Box>
                <Button type="submit" size="300" variant="Primary" radii="400">
                  <Text size="B300">{t('pages:client.explore.change_limit')}</Text>
                </Button>
              </Box>
            </Box>
          </Menu>
              ...
              <Box direction="Column" gap="600">
                ...
                <Box direction="Column" gap="400">
                  <Box direction="Column" gap="300">
                    {isSearch ? (
                      <Text size="H4">{t('pages:client.explore.results_for_term', { term: serverSearchParams.term })}</Text>
                    ) : (
                      <Text size="H4">{t('pages:client.explore.popular_communities')}</Text>
                    )}
                    <Box gap="200">
                    ...
                    (data.chunk.length > 0 ? (
                          ...
                          <Box justifyContent="Center" gap="200">
                            ...
                            >
                              <Text size="B300" truncate>
                                {t('pages:client.explore.previous_page')}
                              </Text>
                            </Button>
                            ...
                            >
                              <Text size="B300" truncate>
                                {t('pages:client.explore.next_page')}
                              </Text>
                            </Button>
                          </Box>
                        )}
                      ...
                      >
                        ...
                        <Text size="T300" align="Center">
                          {t('pages:client.explore.no_communities_found')}
                        </Text>
                      </Box>
                    ))}
                </Box>
