        ...
        >
          <Menu>
            <Header size="300" style={{ padding: `0 ${config.space.S200}` }}>
              <Text size="L400">{t('auth.login.hint.title')}</Text>
            </Header>
            <Box
            ...
            >
              ...
              <Text size="T300">
                <Text as="span" size="Inherit" priority="300">
                  {t('auth.login.hint.matrixId')}
                </Text>
                {` @johndoe:${server}`}
              </Text>
              <Text size="T300">
                <Text as="span" size="Inherit" priority="300">
                  {t('auth.login.hint.email')}
                </Text>
                {` johndoe@${server}`}
              </Text>
            </Box>
        ...
        {loginState.status === AsyncStatus.Error && (
          <>
            {loginState.error.errcode === LoginError.ServerNotAllowed && (
              <FieldError message={t('auth.login.errors.serverNotAllowed')} />
            )}
            {loginState.error.errcode === LoginError.InvalidServer && (
              <FieldError message={t('auth.login.errors.invalidServer')} />
            )}
          </>
        )}
      </Box>
        ...
        <Box alignItems="Start" justifyContent="SpaceBetween" gap="200">
          {loginState.status === AsyncStatus.Error && (
            <>
              {loginState.error.errcode === LoginError.Forbidden && (
                <FieldError message={t('auth.login.errors.invalidCredentials')} />
              )}
              {loginState.error.errcode === LoginError.UserDeactivated && (
                <FieldError message={t('auth.login.errors.accountDeactivated')} />
              )}
              {loginState.error.errcode === LoginError.InvalidRequest && (
                <FieldError message={t('auth.login.errors.invalidRequest')} />
              )}
              {loginState.error.errcode === LoginError.RateLimited && (
                <FieldError message={t('auth.login.errors.rateLimited')} />
              )}
          ...
          <Box grow="Yes" shrink="No" justifyContent="End">
            <Text as="span" size="T200" priority="400" align="Right">
              <Link to={getResetPasswordPath(server)}>{t('auth.login.forgotPassword')}</Link>
            </Text>
          </Box>
        </Box>
      </Box>
