  ...
  return (
    ...
    >
      ...
      <Box direction="Column" gap="100">
        <Text size="L400">{t('auth.login.title')}</Text>
        <Text size="T300">
          <b>{message}</b>
        </Text>
      </Box>
      ...
      {loginState.status === AsyncStatus.Error && (
        <>
          {loginState.error.errcode === LoginError.Forbidden && (
            <LoginTokenError message={t('auth.login.errors.invalidToken')} />
          )}
          {loginState.error.errcode === LoginError.UserDeactivated && (
            <LoginTokenError message={t('auth.login.errors.accountDeactivated')} />
          )}
          {loginState.error.errcode === LoginError.InvalidRequest && (
            <LoginTokenError message={t('auth.login.errors.invalidRequest')} />
          )}
          {loginState.error.errcode === LoginError.RateLimited && (
            <LoginTokenError message={t('auth.login.errors.rateLimited')} />
          )}
