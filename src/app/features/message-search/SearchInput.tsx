  ...
  return (
    <Box as="form" direction="Column" gap="100" onSubmit={handleSearchSubmit}>
      ...
      <Input
        ...
        size="500"
        variant="Background"
        placeholder={t('features:message-search.search_for_keyword')}
        autoComplete="off"
        before={
          active && loading ? (
        ...
        after={
          active ? (
            <Chip
              ...
              onClick={onReset}
            >
              <Text size="B300">{t('features:message-search.clear')}</Text>
            </Chip>
          ) : (
            <Chip type="submit" variant="Primary" size="400" radii="Pill" outlined>
              <Text size="B300">{t('features:message-search.enter')}</Text>
            </Chip>
          )
        }
      />
