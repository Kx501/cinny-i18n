  ...
  return devices.length > 0 ? (
    <>
      <Box direction="Column" gap="100">
        <Text size="L400">{t('features:settings.devices.others')}</Text>
        {authMetadata && (
          <SequenceCard
          ...
          >
            <SettingTile
              title={t('features:settings.devices.device_dashboard')}
              description={t('features:settings.devices.manage_your_devices')}
              after={
                <Button
                  size="300"
                  variant="Secondary"
                ...
                >
                  <Text size="B300">{t('features:settings.devices.open')}</Text>
                </Button>
              }
            />
          </SequenceCard>
        ...
        >
          <Box alignItems="Center" gap="400">
            <Box grow="Yes" direction="Column">
              {deleteError ? (
                <Text size="T200">
                  <b>{t('features:settings.devices.failed_to_logout')} {deleteError.message}</b>
                </Text>
              ) : (
                <Text size="T200">
                  <b>{t('features:settings.devices.logout_from_selected')} ({deleted.size} {t('features:settings.devices.selected')})</b>
                </Text>
              )}
              {authData && (
                <ActionUIAFlowsLoader
                  ...
                  unsupported={() => (
                    <Text size="T200">
                      {t('features:settings.devices.authentication_steps')}
                    </Text>
                  )}
                >
                  {(ongoingFlow) => (
