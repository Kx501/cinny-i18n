    ...
    <Box as="form" onSubmit={handleSubmit} direction="Column" gap="400">
      <Box gap="400">
        <Box grow="Yes" direction="Column" gap="100">
          <Text size="L400">{t('features:common-settings.general.avatar')}</Text>
          {uploadAtom ? (
            ...
            <Box gap="200">
              ...
              >
                <Text size="B300">{t('features:common-settings.general.upload')}</Text>
              </Button>
              {!roomAvatar && avatar && (
                <Button
                  type="button"
                ...
                >
                  <Text size="B300">{t('features:common-settings.general.reset')}</Text>
                </Button>
              )}
              {roomAvatar && (
                <Button
                ...
                >
                  <Text size="B300">{t('features:common-settings.general.remove')}</Text>
                </Button>
              )}
            </Box>
          )}
      ...
      <Box direction="Inherit" gap="100">
        <Text size="L400">{t('features:common-settings.general.name')}</Text>
        <Input
          name="nameInput"
          defaultValue={name}
          variant="Secondary"
      ...
      <Box direction="Inherit" gap="100">
        <Text size="L400">{t('features:common-settings.general.topic')}</Text>
        <TextArea
          name="topicTextArea"
          defaultValue={topic}
          variant="Secondary"
      ...
      <Box gap="300">
        ...
        >
          <Text size="B300">{t('features:common-settings.general.save')}</Text>
        </Button>
        <Button
          type="reset"
          onClick={onClose}
          ...
          radii="300"
        >
          <Text size="B300">{t('features:common-settings.general.cancel')}</Text>
        </Button>
      </Box>
    </Box>
  );
...
export function RoomProfile({ permissions }: RoomProfileProps) {
  ...
  return (
    <Box direction="Column" gap="100">
      <Text size="L400">{t('features:common-settings.general.profile')}</Text>
      <SequenceCard
            ...
            <Box grow="Yes" direction="Column" gap="300">
              <Box direction="Column" gap="100">
                <Text className={BreakWord} size="H5">
                  {name ?? t('features:common-settings.general.unknown')}
                </Text>
                {topic && (
              ...
              {canEdit && (
                <Box gap="200">
                  <Chip
                    ...
                    outlined
                  >
                    <Text size="B300">{t('features:common-settings.general.edit')}</Text>
                  </Chip>
                </Box>
              )}
            </Box>
