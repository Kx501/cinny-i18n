...
export function CreateRoomAliasInput({ disabled }: { disabled?: boolean }) {
  ...
  return (
    <Box shrink="No" direction="Column" gap="100">
      <Text size="L400">{t('components:create-room.address_optional')}</Text>
      <Text size="T200" priority="300">
        {t('components:create-room.pick_an_unique_address')}
      </Text>
      <Input
        ref={aliasInputRef}
      ...
      {aliasAvailable === false && (
        <Box style={{ color: color.Critical.Main }} alignItems="Center" gap="100">
          ...
          <Text size="T200">
            <b>{t('components:create-room.this_address_is_already_taken')}</b>
          </Text>
        </Box>
      )}
    </Box>
