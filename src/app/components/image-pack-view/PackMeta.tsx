        ...
        {canEdit && (
          <Box gap="200">
            <Chip
              ...
              outlined
            >
              <Text size="B300">{t('components:image-pack-view.edit')}</Text>
            </Chip>
          </Box>
        )}
      </Box>
    ...
    <Box as="form" onSubmit={handleSubmit} direction="Column" gap="400">
      <Box gap="400">
        <Box grow="Yes" direction="Column" gap="100">
          <Text size="L400">{t('components:image-pack-view.pack_avatar')}</Text>
          {uploadAtom ? (
            ...
            <Box gap="200">
              ...
              >
                <Text size="B300">{t('components:image-pack-view.upload')}</Text>
              </Button>
              {!avatar && meta.avatar && (
                <Button
                  type="button"
      ...
      <Box direction="Inherit" gap="100">
        <Text size="L400">{t('components:image-pack-view.name')}</Text>
        <Input name="nameInput" defaultValue={meta.name} variant="Secondary" radii="300" required />
      </Box>
      <Box direction="Inherit" gap="100">
        <Text size="L400">{t('components:image-pack-view.attribution')}</Text>
        <TextArea
      ...
      <Box gap="300">
        <Button type="submit" variant="Success" size="300" radii="300" disabled={uploadingAvatar}>
          <Text size="B300">{t('components:image-pack-view.save')}</Text>
        </Button>
        <Button
          type="reset"
          onClick={onCancel}
          ...
          radii="300"
        >
          <Text size="B300">{t('components:image-pack-view.cancel')}</Text>
        </Button>
      </Box>
    </Box>
  );
